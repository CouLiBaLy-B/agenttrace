"""Async example: same research agent as sync_basic.py, driven with
`agent.ainvoke()` — the middleware's async hooks (`awrap_model_call`,
`awrap_tool_call`) fire automatically, no extra wiring needed.
...
async def main() -> None:
    agent = create_deep_agent(
        ...
        middleware=[AgentTraceMiddleware(run_name="async example — Rust web frameworks")],
    )
